...
const PersonalizedReport2025 = () => {
  ...
  const BC_PRICE_NUMBER = getRawPrice(PRICE_KEYS.personalizedReport2025);
  const BC_PRICE_FORMATTED = getFormattedPrice(PRICE_KEYS.personalizedReport2025);
  ...
  const sendAbandonmentEmail = useCallback(async (reason = 'Payment cancelled by user') => {
    ...
    try {
      const abandonmentData = {
        ...
        service: 'PersonalizedReport2025',
        birthDetails: {
    ...
    try {
      ...
      if (verifyResult.success) {
        ...
        const astroEmailData = {
          ...
          phone: formData.phone,
          service: 'PersonalizedReport2025',
          reportType: 'PersonalizedReport2025',
          birthDetails: {
            dateOfBirth: formData.dateOfBirth,
            timeOfBirth: formData.timeOfBirth,
            placeOfBirth: formData.placeOfBirth,
          ...
          language: formData.language,
          additionalInfo: 'Complete Personalized Report 2025 Analysis Request',
          paymentDetails: {
            status: 'paid',
        ...
        if (emailResult.success) {
          ...
          setAnalysisData({
            ...
            service: 'Personalized Report 2025',
  ...
  const initializePayment = async (orderData) => {
    ...
    const options = {
      ...
      name: 'SriAstroVeda',
      description: 'Personalized Report 2025 - Complete Kundli Report',
      image: '/logo192.png',
    ...
    try {
      const orderResponse = await fetch(`${API_URL}/create-order`, {
        ...
        body: JSON.stringify({
          amount: BC_PRICE_NUMBER,
          currency: 'INR',
          receipt: `personalized_report_2025_${Date.now()}`,
          notes: {
            service: 'personalized_report_2025_analysis',
    ...
    <div>
      {showThankYou ? (
        <ThankYouPage 
          ...
          serviceFeatures={[
            t('detailed_personalized_report_2025_pdf') || "Detailed Personalized Report 2025 (PDF)",
                    ...
                    <form onSubmit={handleGenerateAnalysis} className="space-y-4 sm:space-y-6">
                      ...
                      <div>
                        ...
                        <div className="bg-gradient-to-r from-purple-400/10 to-pink-400/10 border-2 border-purple-400/50 rounded-lg p-4 sm:p-6">
                          <div className="text-center">
                            <h4 className="text-lg sm:text-xl font-semibold text-white mb-2">{t('complete_personalized_report_2025')}</h4>
                            ...
                            <ul className="text-gray-300 text-xs sm:text-sm space-y-2 text-left max-w-sm mx-auto">
                              ...
                              <li className="flex items-center gap-2">
                                <span className="text-purple-400 flex-shrink-0">✓</span>
                                <span>{t('yearly_predictions')}</span>
                              </li>
                              <li className="flex items-center gap-2">
                                <span className="text-purple-400 flex-shrink-0">✓</span>
                  ...
                  {!showAnalysis ? (
                    <>
                      ...
                      <div className="bg-black/60 backdrop-blur-md rounded-2xl p-4 sm:p-6 lg:p-8 border border-purple-700/50">
                        <div className="relative">
                          <img
                            src={birthChartImage}
                            alt={t('sample_personalized_report_2025')}
                    ...
                    <div className="bg-black/60 backdrop-blur-md rounded-2xl p-4 sm:p-6 lg:p-8 border border-purple-700/50">
                      <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 sm:mb-6 flex items-center">
                        <span className="text-purple-400 mr-2 sm:mr-3 text-lg sm:text-xl">📊</span>
                        {t('your_personalized_report_2025_status')}
                      </h3>
                      <div className="text-center">
