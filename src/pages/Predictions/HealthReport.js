...
const HealthReport = () => {
  ...
  const BC_PRICE_NUMBER = getRawPrice(PRICE_KEYS.healthReport);
  const BC_PRICE_FORMATTED = getFormattedPrice(PRICE_KEYS.healthReport);
    ...
    try {
      ...
      if (verifyResult.success) {
        ...
        const astroEmailData = {
          ...
          language: formData.language,
          additionalInfo: 'Complete Health Index (Kundli) Analysis Request',
          paymentDetails: {
            status: 'paid',
  ...
  const initializePayment = async (orderData) => {
    ...
    const options = {
      ...
      name: 'SriAstroVeda',
      description: 'Health Index Analysis - Complete Kundli Report',
      image: '/logo192.png',
    ...
    try {
      const orderResponse = await fetch(`${API_URL}/create-order`, {
        ...
        body: JSON.stringify({
          amount: BC_PRICE_NUMBER,
          currency: 'INR',
          receipt: `health_report_${Date.now()}`,
          notes: {
            service: 'health_report_analysis',
    ...
    <div>
      {showThankYou ? (
        <ThankYouPage 
          ...
          serviceFeatures={[
            t('detailed_health_report_pdf') || "Detailed Health Index (PDF)",
          ...
          <div className="relative z-10 py-8 sm:py-16 px-4 sm:px-6 lg:px-8">
            ...
            <div className="max-w-7xl mx-auto text-center mb-8 sm:mb-16">
              <h1 className="text-3xl sm:text-4xl lg:text-6xl font-bold text-white mb-4 sm:mb-6 leading-tight">
                {t('create_your')} <span className="text-purple-400">{t('health_report')}</span>
              </h1>
                          ...
                          <div className="text-center">
                            ...
                            <ul className="text-gray-300 text-xs sm:text-sm space-y-2 text-left max-w-sm mx-auto">
                              <li className="flex items-center gap-2">
                                ...
                                <span>{t('detailed_health_report')}</span>
                              </li>
                              <li className="flex items-center gap-2">
                                <span className="text-purple-400 flex-shrink-0">✓</span>
                              ...
                              <li className="flex items-center gap-2">
                                <span className="text-purple-400 flex-shrink-0">✓</span>
                                <span>{t('health_predictions')}</span>
                              </li>
                              <li className="flex items-center gap-2">
                                <span className="text-purple-400 flex-shrink-0">✓</span>
