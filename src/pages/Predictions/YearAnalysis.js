...
const YearAnalysis = () => {
  ...
  const BC_PRICE_NUMBER = getRawPrice(PRICE_KEYS.yearAnalysis);
  const BC_PRICE_FORMATTED = getFormattedPrice(PRICE_KEYS.yearAnalysis);
    ...
    try {
      ...
      if (verifyResult.success) {
        ...
        const astroEmailData = {
          ...
          language: formData.language,
          additionalInfo: 'Complete Year Analysis Report Request',
          paymentDetails: {
            status: 'paid',
  ...
  const initializePayment = async (orderData) => {
    ...
    const options = {
      ...
      name: 'SriAstroVeda',
      description: 'Year Analysis Report - Complete Kundli Report',
      image: '/logo192.png',
    ...
    try {
      const orderResponse = await fetch(`${API_URL}/create-order`, {
        ...
        body: JSON.stringify({
          amount: BC_PRICE_NUMBER,
          currency: 'INR',
          receipt: `year_analysis_${Date.now()}`,
          notes: {
            service: 'year_analysis_analysis',
    ...
    <div>
      {showThankYou ? (
        <ThankYouPage 
          ...
          serviceFeatures={[
            t('detailed_year_analysis_pdf') || "Detailed Year Analysis (PDF)",
          ...
          <div className="relative z-10 py-8 sm:py-16 px-4 sm:px-6 lg:px-8">
            ...
            <div className="max-w-7xl mx-auto text-center mb-8 sm:mb-16">
              <h1 className="text-3xl sm:text-4xl lg:text-6xl font-bold text-white mb-4 sm:mb-6 leading-tight">
                {t('create_your')} <span className="text-purple-400">{t('year_analysis')}</span>
