...
const Three = () => {
  ...
  // Autoplay: advance one card every 2s (paused on hover/drag or if user prefers reduced motion)
  useEffect(() => {
    const track = trackRef.current;
    if (!track) return;
    let timer = null;
  ...
  // NEW: Prime-load only the first 3 videos after 3 seconds on first open (defer everything else)
  useEffect(() => {
    const track = trackRef.current;
    if (!track) return;
    const timeoutId = setTimeout(() => {
      const cards = track.querySelectorAll('[data-card]');
      const firstThree = Array.from(cards).slice(0, 3);
      firstThree.forEach((card) => {
        const video = card.querySelector('video');
        if (!video) return;
        if (!video.src && video.dataset.src) {
          // attach src and load metadata to keep cost low
          ...
          >
            {zodiacData.map((zodiac) => (
              ...
              >
                ...
                {/* Video (lazy src attach; default preload=none) */}
