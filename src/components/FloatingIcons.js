...
// A fixed floating action button shown on the bottom-right across the site
// Only WhatsApp is shown; falls back to /contact if number missing
const FloatingIcons = ({ whatsappNumber = '' }) => {
  ...
  <div className="fixed right-4 bottom-4 z-50 flex flex-col items-center gap-3 pointer-events-auto">
      ...
      <a
        ...
        {...ext(waLink)}
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-6 h-6 fill-current animate-bounce motion-reduce:animate-none">
