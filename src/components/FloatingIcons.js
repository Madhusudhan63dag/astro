...
const FloatingIcons = ({ whatsappNumber = '', phoneNumber = '', email = '' }) => {
  ...
  // Build deep link + web fallback (desktop/Web)
  const waDeep = waDigits ? `whatsapp://send?phone=${waDigits}` : '/contact';
  const waWeb  = waDigits ? `https://wa.me/${waDigits}` : '/contact';

  const phoneLink = phoneNumber ? `tel:${phoneNumber}` : '/contact';
  const mailLink  = email ? `mailto:${email}` : '/contact';

  // Only add target for generic external http(s) links, not for WhatsApp deep/web links
  const ext = (href) => {
    const isHttp = /^https?:\/\//.test(href);
    const isWa   = /wa\.me\/|api\.whatsapp|^whatsapp:\/\//i.test(href);
    return isHttp && !isWa ? { target: '_blank', rel: 'noopener noreferrer' } : {};
  };

  const handleWhatsAppClick = (e) => {
    if (!waDigits) return; // will go to /contact
    // Attempt native deep link on mobile, fallback to wa.me if it doesn't open quickly
    const ua = (typeof navigator !== 'undefined' ? navigator.userAgent : '') || '';
    const isMobile = /android|iphone|ipad|ipod/i.test(ua);
    if (!isMobile) return; // let desktop use WhatsApp Web via href
    ...
    // Fallback to wa.me if deep link didn't take effect within ~800ms–1s
    setTimeout(() => {
      // Still in the same page shortly after suggests deep link failed
  ...
  return (
    <div className="fixed right-4 bottom-4 z-50 flex flex-col items-center gap-3 pointer-events-auto">
      ...
      <a
        ...
        // No target for wa.me; let the OS/app handle it in-place
      ...
      {/* Phone (keeps default behavior) */}
      <a href={phoneLink} aria-label="Call" title="Call" className="hidden" {...ext(phoneLink)} />

      {/* Email (keeps default behavior) */}
      <a href={mailLink} aria-label="Email" title="Email" className="hidden" {...ext(mailLink)} />
    </div>
  );
};
